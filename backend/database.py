...
class Database:
    ...
    def archive_old_history(self, retention_days: int = 365) -> int:
        ...
        """, (retention_days,))
        archived = c.rowcount
        c.execute("DELETE FROM history WHERE closed_price IS NOT NULL AND date <= date('now', '-' || ? || ' days')", (retention_days,))
        conn.commit()
        conn.close()
        return archived
