...
def build_outlook(conn, symbol: str, date: str) -> dict:
    ...
    # ── Adaptive market classification (condition-driven decision engine) ──
    try:
        from adaptive_engine import classify_market_structure, _safe as _a_safe
        _ind = ind if ind else {}
        _v = vix.get("close") if vix.get("close") else None
        _gap_available = bool(gap.get("available"))
        _data_for_adaptive = {
            "symbol": symbol,
            "price": _a_safe(ind.get("day_high") or day.get("close") or ind.get("close")),
            "ema20": _a_safe(ind.get("ema20")),
            "ema50": _a_safe(ind.get("ema50")),
            "ema200": None,
            "vwap": _a_safe(ind.get("vwap")),
            "adx": _a_safe(ind.get("adx")),
            "rsi": _a_safe(ind.get("rsi")),
            "macd": _a_safe(ind.get("macd")),
            "macd_signal": None,
            "cpr_type": "NEUTRAL",
            "cpr_value": None,
            "vix": _v,
            "gap_available": _gap_available,
            "gap_pct": gap.get("gap_pct") if _gap_available else None,
            "gap_fill_pct": gap.get("fill_pct") if _gap_available else None,
            "breadth_ratio": None,
            "support": [_a_safe(s) for s in (ind.get("support_resistance") or {}).get("support", []) if isinstance(ind.get("support_resistance"), dict)],
            "resistance": [_a_safe(s) for s in (ind.get("support_resistance") or {}).get("resistance", []) if isinstance(ind.get("support_resistance"), dict)],
            "data_state": "LIVE",
            "options_available": bool(pcr.get("available", False)) if isinstance(pcr, dict) else False,
        }
        if isinstance(ind.get("support_resistance"), dict):
            _sr = ind["support_resistance"]
            if isinstance(_sr, dict):
                _data_for_adaptive["support"] = [_a_safe(x) for x in _sr.get("support", [])]
                _data_for_adaptive["resistance"] = [_a_safe(x) for x in _sr.get("resistance", [])]
        _classified = classify_market_structure(_data_for_adaptive)
        payload["market_structure"] = _classified["market_structure"]
        payload["directional_bias"] = _classified["directional_bias"]
        payload["trade_class"] = _classified["trade_class"]
        payload["trade_status"] = _classified["trade_status"]
        payload["entry_trigger"] = _classified["entry_trigger"]
        payload["confirmation_conditions"] = _classified["confirmation_conditions"]
        payload["invalidation"] = _classified["invalidation"]
        payload["target_zone"] = _classified["target_zone"]
        payload["preferred_strategy"] = _classified["preferred_strategy"]
        payload["supporting_factors"] = _classified["supporting_factors"]
        payload["conflicting_factors"] = _classified["conflicting_factors"]
        payload["adaptive_confidence"] = _classified["confidence"]
        payload["data_state"] = _classified["data_state"]
    except Exception:
        payload.setdefault("market_structure", regime_label)
        payload.setdefault("directional_bias", bias_label)
        payload.setdefault("trade_class", "DIRECTIONAL" if verdict == "TRADE" else "NO_TRADE")
        payload.setdefault("trade_status", "WAIT" if verdict == "WAIT" else "ACTIVE")
        payload.setdefault("entry_trigger", "See decision section")
        payload.setdefault("confirmation_conditions", [])
        payload.setdefault("invalidation", "Key level breach")
        payload.setdefault("target_zone", "Next resistance zone")
        payload.setdefault("preferred_strategy", "Monitor")
        payload.setdefault("supporting_factors", [])
        payload.setdefault("conflicting_factors", [])
        payload.setdefault("adaptive_confidence", conf)
