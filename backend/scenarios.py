...
class ScenarioEngine:
    def generate(self, regime: str, support_levels: list, resistance_levels: list, current_price: float, adx: Optional[float] = None, vix_price: float = 0) -> dict[str, Any]:
        ...
        bullish = {"trigger": "Price breaks above resistance", "confirmation": "5-min close above resistance + volume", "target": resistance_levels[0] if resistance_levels else "N/A", "invalidation": "Below pivot", "strategy_environment": "BULLISH", "probability": 0.3}
        bearish = {"trigger": "Price breaks below support", "confirmation": "5-min close below support + volume", "target": support_levels[-1] if support_levels else "N/A", "invalidation": "Above pivot", "strategy_environment": "BEARISH", "probability": 0.3}
