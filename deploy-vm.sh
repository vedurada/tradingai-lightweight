...
sudo cp $PROJECT_DIR/ops/systemd/tradingai-api.service /etc/systemd/system/tradingai-api.service && sudo systemctl daemon-reload && sudo systemctl enable tradingai-api && sudo systemctl restart tradingai-api && if bash $PROJECT_DIR/ops/health_gate.sh; then echo "DEPLOY SUCCESS: API healthy"; deploy_log "VERIFY" "PASS" "API healthy"; else echo "DEPLOY FAILED: API health check failed"; exit 1; fi"
