...
ssh -i "$SSH_KEY" -o StrictHostKeyChecking=no "$VM_USER@$VM_HOST" "pkill -f api_server.py 2>/dev/null; sleep 1; cd $PROJECT_DIR/backend && /usr/bin/python3 db_schema.py && (SKIP_LLM=1 /usr/bin/python3 data_fetcher_db.py >> /opt/tradingai/logs/data.log 2>&1 &) && sleep 2; cd $PROJECT_DIR/backend && setsid nohup /usr/bin/python3 api_server.py > /opt/tradingai/logs/api_server.log 2>&1 < /dev/null & sleep 3; curl -s http://127.0.0.1:8000/api/health"

# Every-minute market-hours fetch (tiered inside fetcher: indices+large every run, mid 5m, small+fundamentals 15m)
ssh -i "$SSH_KEY" -o StrictHostKeyChecking=no "$VM_USER@$VM_HOST" "(crontab -l 2>/dev/null | grep -v 'data_fetcher_db.py\|generate_data.py\|generate_json.py\|api_server.py'; echo '# TradingAI Market Hours Cron (9:30 AM - 3:30 PM IST, Mon-Fri)'; echo '* 9-15 * * 1-5 cd /opt/tradingai/backend && SKIP_LLM=1 /usr/bin/python3 data_fetcher_db.py >> /opt/tradingai/logs/data.log 2>&1'; echo '*/5 9-15 * * 1-5 cd /opt/tradingai/backend && /usr/bin/python3 monitor.py >> /opt/tradingai/logs/monitor.log 2>&1'; echo '0 */2 9-15 * * 1-5 cd /opt/tradingai/backend && /usr/bin/python3 alert.py >> /opt/tradingai/logs/alerts.log 2>&1'; echo '*/5 * * * * pgrep -f api_server.py > /dev/null || (cd /opt/tradingai/backend && setsid nohup /usr/bin/python3 api_server.py > /opt/tradingai/logs/api_server.log 2>&1 < /dev/null &)'; echo '@reboot cd /opt/tradingai/backend && setsid nohup /usr/bin/python3 api_server.py > /opt/tradingai/logs/api_server.log 2>&1 < /dev/null &') | crontab - && crontab -l"
