...
ssh -i "$SSH_KEY" -o StrictHostKeyChecking=no "$VM_USER@$VM_HOST" "pkill -f '[p]ython3 api_server' 2>/dev/null; pkill -f '[g]unicorn' 2>/dev/null; sleep 1; mkdir -p /opt/tradingai/logs && cd $PROJECT_DIR/backend && /usr/bin/python3 db_schema.py && (SKIP_LLM=1 /usr/bin/python3 data_fetcher_db.py >> /opt/tradingai/logs/data.log 2>&1 &); (sleep 10 && if [ \$(/usr/bin/python3 -c 'import sqlite3; c=sqlite3.connect(\"/opt/tradingai/database/tradingai.db\"); print(c.execute(\"SELECT COUNT(*) FROM price_1d WHERE symbol=\"\"NIFTY\"\"\").fetchone()[0] if c else 0)' 2>/dev/null) -lt 500 ]; then /usr/bin/python3 backfill_indices_10y.py --period 10y >> /opt/tradingai/logs/data.log 2>&1 & fi &); (sleep 20 && if [ \$(/usr/bin/python3 -c 'import sqlite3; c=sqlite3.connect(\"/opt/tradingai/database/tradingai.db\"); print(c.execute(\"SELECT COUNT(*) FROM market_outlooks\").fetchone()[0] if c else 0)' 2>/dev/null) -lt 500 ]; then /usr/bin/python3 backfill_outlooks.py --days 3650 --overwrite >> /opt/tradingai/logs/outlook.log 2>&1 & fi &); sudo cp $PROJECT_DIR/ops/systemd/tradingai-api.service /etc/systemd/system/tradingai-api.service && sudo systemctl daemon-reload && sudo systemctl enable --now tradingai-api && sleep 4; curl -s http://127.0.0.1:8000/api/health || true"
