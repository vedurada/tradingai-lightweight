...
if ssh -i "$SSH_KEY" -o StrictHostKeyChecking=no "$VM_USER@$VM_HOST" "pkill -f '[p]ython3 api_server' 2>/dev/null; pkill -f '[g]unicorn' 2>/dev/null; sleep 1; mkdir -p /opt/tradingai/logs && cd $PROJECT_DIR/backend && /usr/bin/python3 db_schema.py && (SKIP_LLM=1 /usr/bin/python3 data_fetcher_db.py >> /opt/tradingai/logs/data.log 2>&1 &); (sleep 10 && CNT=\$(/usr/bin/python3 -c 'import sqlite3; print(sqlite3.connect(\"/opt/tradingai/database/tradingai.db\").execute(\"SELECT COUNT(*) FROM price_1d\").fetchone()[0])' 2>/dev/null || echo SKIP); if [ \"\$CNT\" != SKIP ] && [ \"\$CNT\" -lt 500 ]; then /usr/bin/python3 backfill_indices_10y.py --period 10y >> /opt/tradingai/logs/data.log 2>&1 & fi &); (sleep 20 && CNT=\$(/usr/bin/python3 -c 'import sqlite3; print(sqlite3.connect(\"/opt/tradingai/database/tradingai.db\").execute(\"SELECT COUNT(*) FROM market_outlooks\").fetchone()[0])' 2>/dev/null || echo SKIP); if [ \"\$CNT\" != SKIP ] && [ \"\$CNT\" -lt 500 ]; then /usr/bin/python3 backfill_outlooks.py --days 3650 --overwrite >> /opt/tradingai/logs/outlook.log 2>&1 & fi &); # Health gate: api/health must report '"status": "ok"'. ops/health_gate.sh polls
# with retries (12x5s) as a script file to avoid SSH inline-quoting fragility.
sudo cp $PROJECT_DIR/ops/systemd/tradingai-api.service /etc/systemd/system/tradingai-api.service && sudo systemctl daemon-reload && sudo systemctl enable tradingai-api && sudo systemctl restart tradingai-api && bash $PROJECT_DIR/ops/health_gate.sh"
then echo "DEPLOY SUCCESS: API healthy"; deploy_log "VERIFY" "PASS" "API healthy"; else echo "DEPLOY FAILED: API health check failed"; exit 1; fi

# Track C S2: pre-render snapshot first paint now that the API is healthy.
# `; true` because prerender exits 3 (SKIP) when the API is unreachable — a stale
# first paint must never fail the deploy. Script itself is idempotent.
ssh -i "$SSH_KEY" -o StrictHostKeyChecking=no "$VM_USER@$VM_HOST" "/usr/bin/python3 /opt/tradingai/ops/prerender_snapshot.py --root /var/www/tradingai.in/html --api-base http://127.0.0.1:8000 >> /opt/tradingai/logs/prerender.log 2>&1; true"

# Every-minute market-hours fetch (tiered inside fetcher: indices+large every run, mid 5m, small+fundamentals 15m)
ssh -i "$SSH_KEY" -o StrictHostKeyChecking=no "$VM_USER@$VM_HOST" "(crontab -l 2>/dev/null | grep -v 'data_fetcher_db.py\|generate_data.py\|generate_json.py\|api_server.py\|pnl_tracker.py\|vm-backup.sh\|self-heal.sh\|aggregate.py\|nse_live_chain.py\|etf_fetcher.py\|mf_fetcher.py\|outlook.py\|sitemap_gen.py\|bhavcopy.py\|backfill_yearly.py\|fo_fetcher.py\|daily_page.py\|monitor.py\|alert.py'; echo '# TradingAI Market Hours Cron (9:30 AM - 3:30 PM IST, Mon-Fri)'; echo '* 9-15 * * 1-5 cd /opt/tradingai/backend && SKIP_LLM=1 /usr/bin/python3 data_fetcher_db.py >> /opt/tradingai/logs/data.log 2>&1'; echo '*/5 9-15 * * 1-5 cd /opt/tradingai/backend && /usr/bin/python3 monitor.py >> /opt/tradingai/logs/monitor.log 2>&1'; echo '0 */2 9-15 * * 1-5 cd /opt/tradingai/backend && /usr/bin/python3 alert.py >> /opt/tradingai/logs/alerts.log 2>&1'; echo '*/5 9-15 * * 1-5 cd /opt/tradingai/backend && /usr/bin/python3 pnl_tracker.py evaluate >> /opt/tradingai/logs/pnl.log 2>&1'; echo '20 15 * * 1-5 cd /opt/tradingai/backend && /usr/bin/python3 pnl_tracker.py close >> /opt/tradingai/logs/pnl.log 2>&1'; echo '35 9 * * 1-5 cd /opt/tradingai/backend && /usr/bin/python3 daily_page.py morning --webroot /var/www/tradingai.in/html >> /opt/tradingai/logs/daily.log 2>&1'; echo '35 15 * * 1-5 cd /opt/tradingai/backend && /usr/bin/python3 daily_page.py close --webroot /var/www/tradingai.in/html && /usr/bin/python3 sitemap_gen.py --webroot /var/www/tradingai.in/html >> /opt/tradingai/logs/daily.log 2>&1'; echo '30 18 * * * /opt/tradingai/ops/vm-backup.sh >> /opt/tradingai/logs/backup.log 2>&1'; echo '0 2 1 * * cd /opt/tradingai/backend && /usr/bin/python3 pnl_tracker.py archive 365 >> /opt/tradingai/logs/pnl.log 2>&1'; echo '35 18 * * 1-5 cd /opt/tradingai/backend && /usr/bin/python3 bhavcopy.py daily >> /opt/tradingai/logs/bhav.log 2>&1'; echo '30 8 * * 1 cd /opt/tradingai/backend && /usr/bin/python3 bhavcopy.py holidays >> /opt/tradingai/logs/bhav.log 2>&1'; echo '30 6 * * 0 cd /opt/tradingai/backend && /usr/bin/python3 backfill_yearly.py >> /opt/tradingai/logs/data.log 2>&1'; echo '40 18 * * 1-5 cd /opt/tradingai/backend && /usr/bin/python3 fo_fetcher.py daily >> /opt/tradingai/logs/fo.log 2>&1'; echo '30 9 * * 1-5 cd /opt/tradingai/backend && . /etc/tradingai/groq.env && { /usr/bin/python3 outlook.py --symbol NIFTY --webroot /var/www/tradingai.in/html && /usr/bin/python3 outlook.py --symbol BANKNIFTY --webroot /var/www/tradingai.in/html && /usr/bin/python3 outlook.py --symbol FINNIFTY --webroot /var/www/tradingai.in/html && /usr/bin/python3 outlook.py --symbol SENSEX --webroot /var/www/tradingai.in/html && /usr/bin/python3 /opt/tradingai/ops/inject_trust_headers.py --root /var/www/tradingai.in/html && /usr/bin/python3 /opt/tradingai/ops/prerender_snapshot.py --root /var/www/tradingai.in/html >> /opt/tradingai/logs/prerender.log 2>&1; } >> /opt/tradingai/logs/outlook.log 2>&1'; echo '0 19 * * 1-5 cd /opt/tradingai/backend && . /etc/tradingai/groq.env && { /usr/bin/python3 outlook.py --symbol NIFTY --webroot /var/www/tradingai.in/html && /usr/bin/python3 outlook.py --symbol BANKNIFTY --webroot /var/www/tradingai.in/html && /usr/bin/python3 outlook.py --symbol FINNIFTY --webroot /var/www/tradingai.in/html && /usr/bin/python3 outlook.py --symbol SENSEX --webroot /var/www/tradingai.in/html && /usr/bin/python3 sitemap_gen.py --webroot /var/www/tradingai.in/html && /usr/bin/python3 /opt/tradingai/ops/inject_trust_headers.py --root /var/www/tradingai.in/html && /usr/bin/python3 /opt/tradingai/ops/prerender_snapshot.py --root /var/www/tradingai.in/html >> /opt/tradingai/logs/prerender.log 2>&1; } >> /opt/tradingai/logs/outlook.log 2>&1'; echo '35 19 * * * cd /opt/tradingai/backend && /usr/bin/python3 mf_fetcher.py >> /opt/tradingai/logs/mf.log 2>&1'; echo '30 7 * * 0 cd /opt/tradingai/backend && /usr/bin/python3 mf_fetcher.py --returns >> /opt/tradingai/logs/mf.log 2>&1'; echo '*/2 * * * * /opt/tradingai/ops/self-heal.sh >> /opt/tradingai/logs/self-heal.log 2>&1' echo '*/15 9-15 * * 1-5 cd /opt/tradingai/backend && /usr/bin/python3 aggregate.py sweep >> /opt/tradingai/logs/aggregate.log 2>&1'; echo '*/15 9-15 * * 1-5 cd /opt/tradingai/backend && /usr/bin/python3 nse_live_chain.py poll >> /opt/tradingai/logs/livechain.log 2>&1'; echo '0 7 * * 0 cd /opt/tradingai/backend && /usr/bin/python3 etf_fetcher.py >> /opt/tradingai/logs/etf.log 2>&1'; echo '0 3 * * * /opt/tradingai/ops/cleanup.sh >> /opt/tradingai/logs/cleanup.log 2>&1') | crontab - && crontab -l"
