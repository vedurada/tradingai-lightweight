...
ssh_vm "if [ ! -f $PROJECT_DIR/database/tradingai.db ]; then rm -rf /tmp/tradingai-restore && git clone -q --branch vm-backup --depth 1 https://github.com/vedurada/tradingai-lightweight.git /tmp/tradingai-restore 2>/dev/null && ( if [ -f /tmp/tradingai-restore/database/tradingai.db.gz ]; then gzip -dc /tmp/tradingai-restore/database/tradingai.db.gz > $PROJECT_DIR/database/tradingai.db && echo RESTORED-DATABASE-OK-GZ; elif [ -f /tmp/tradingai-restore/database/tradingai.db ]; then cp /tmp/tradingai-restore/database/tradingai.db $PROJECT_DIR/database/tradingai.db && echo RESTORED-DATABASE-OK; else echo NO-DB-FILE-IN-BACKUP; fi ) || echo 'NO-BACKUP-BRANCH-YET (fresh database will be created by fetcher)'; else echo 'database already present, kept as-is'; fi"

echo "=== 5/8 web root content ==="
ssh_vm "cp $PROJECT_DIR/*.html $WEB_ROOT/html/ 2>/dev/null; cp -r $PROJECT_DIR/indices/* $WEB_ROOT/html/indices/ 2>/dev/null; cp -r $PROJECT_DIR/stocks/* $WEB_ROOT/html/stocks/ 2>/dev/null; cp $PROJECT_DIR/static/css/*.css $WEB_ROOT/html/assets/css/ 2>/dev/null; cp $PROJECT_DIR/static/js/*.js $WEB_ROOT/html/assets/js/ 2>/dev/null; mkdir -p $WEB_ROOT/html/data; echo WEBROOT-OK"
