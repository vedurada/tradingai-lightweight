...
def ingest(instrument_id, days=55, force=False):
    ...
    report_path = f"/opt/tradingai_new/data/generated/{instrument_id.lower()}_ingestion_quality.json"
...
def session_analysis(instrument_id):
    ...
    report_path = f"/opt/tradingai_new/data/generated/{instrument_id.lower()}_session_quality.json"
    os.makedirs(os.path.dirname(report_path), exist_ok=True)
    with open(report_path, 'w') as f:
