...
<footer>TradingAI.in · Smarter Analysis. Disciplined Trading. · Paper feed is experimental. · <a href="/privacy.html" style="color:#94a3b8">Privacy</a> · <a href="/contact.html" style="color:#94a3b8">Contact</a> · <a href="/terms.html" style="color:#94a3b8">Terms</a></footer>
<script>
(function(){{
...
async function loadLive(){{
  ...
  try{{
    var cards='';
    for(const inst of ['NIFTY','BANKNIFTY']){{
      let r=await fetch('/api/paper/spread/today?instrument='+inst.toLowerCase(),{{cache:'no-store'}});
      if(!r.ok)continue;
      const d=(await r.json()).data||{{}};
      const tg=d.trigger||{{}}, sp=d.spread||{{}};
      const legs=(sp.legs||[]).map(l=>esc(l.side)+' '+esc(l.type)+' '+esc(l.strike)).join(' / ')||'—';
      cards+='<div class="card"><h3>'+inst+' live</h3><div class="kv">'
