async function loadBankNifty() {
  const data = await fetchJSON('banknifty');
  if (!data) return;
  ...
  if (historyEl) {
    ...
    if (histData && histData.BANKNIFTY) {
      const entries = histData.BANKNIFTY.slice(0, 7);
  ...
  if (outlookHistEl) {
    const histData = await fetchJSON('history');
    if (histData && histData.BANKNIFTY) {
      const entries = histData.BANKNIFTY.slice(0, 5);
...
document.addEventListener('DOMContentLoaded', () => { loadBankNifty(); startDataRefresh(loadBankNifty); });