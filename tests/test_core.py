...
class TestStrategyEngine(unittest.TestCase):
    def test_bullish_is_credit_only(self):
        s = StrategyEngine()
        for vol in ("HIGH", "NORMAL", "LOW"):
            r = s.select_strategy("BULLISH_CONTINUATION", "BULLISH", vol)
            self.assertEqual(r["strategy"], "Bull Put Spread")

    def test_bearish_is_credit_only(self):
        s = StrategyEngine()
        for vol in ("HIGH", "NORMAL", "LOW"):
            r = s.select_strategy("BEARISH_CONTINUATION", "BEARISH", vol)
...
class TestTradeDefinition(unittest.TestCase):
    def test_trade_has_required_fields(self):
        ...
        for t in result['trades']:
            ...
            self.assertIn('target_price', t)
            self.assertIn('trade_date', t)

    def test_trade_entry_gt_stop(self):
        engine = BacktestEngine()
        result = engine.run('NIFTY', '2026-09-12', '2026-09-19')
        for t in result['trades']:
            self.assertGreater(t['entry_price'], t['stop_price'], f"Entry should be above stop for {t['trade_id']}")

    def test_trade_target_gt_entry(self):
        engine = BacktestEngine()
        result = engine.run('NIFTY', '2026-09-12', '2026-09-19')
        for t in result['trades']:
            self.assertGreater(t['target_price'], t['entry_price'], f"Target should be above entry for {t['trade_id']}")
...
class TestMultipleSignalsSameDay(unittest.TestCase):
    def test_multiple_candles_same_day(self):
        ...
        trades = result['trades']
        self.assertLessEqual(len(trades), 1, f"One day should have max 1 trade, got {len(trades)}")
        days = set(t['trade_date'] for t in trades)
...
class TestTradeLineage(unittest.TestCase):
    def test_trade_has_session_date(self):
        ...
        result = engine.run('NIFTY', '2026-09-12', '2026-09-19')
        for t in result['trades']:
            self.assertIsNotNone(t.get('trade_date'))
...
class TestEntryTiming(unittest.TestCase):
    def test_entry_before_exit(self):
        ...
        for t in result['trades']:
            ...
            exit_date = str(t['exit_time'])[:10] if isinstance(t.get('exit_time'), str) else ''
    ...
    def test_entry_from_historical_candle(self):
        ...
        for t in result['trades']:
            candle = conn.execute("SELECT close FROM market_candles_5m WHERE instrument_id='NIFTY' AND timestamp <= ? AND timestamp >= ? ORDER BY timestamp DESC LIMIT 1", (t['trade_date'] + 'T23:59:59', t['trade_date'] + 'T00:00:00')).fetchone()
            self.assertIsNotNone(candle, f"No candle found for {t['trade_date']}")
        conn.close()
