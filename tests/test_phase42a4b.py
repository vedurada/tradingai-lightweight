...
TEST_DB = os.path.join(tempfile.gettempdir(), f"tradingai_test_phase42a4b_{int(time.time())}.db")
...
class TestSetupIdentity:
    ...
    def test_record_setup_identity(self):
        ...
        result = collector.record_setup_identity(instrument="NIFTY", candle_timestamp=_unique_ts(), direction="BULLISH", regime="BULLISH", trade_state="TRADE", strategy="CALL_DEBIT_SPREAD", evidence_summary={"trend": "BULLISH"}, ai_outlook_id="OUT-TEST")
    ...
    def test_setup_id_contains_required_fields(self):
        collector = get_collector()
        ts = _unique_ts()
        result = collector.record_setup_identity(instrument="NIFTY", candle_timestamp=ts, direction="BULLISH", regime="BULLISH", trade_state="TRADE", strategy="CALL_DEBIT_SPREAD")
...
class TestReentryInstrumentation:
    ...
    def test_record_reentry(self):
        collector = get_collector()
        result = collector.record_reentry(trade_id="PT-TEST-001", instrument="NIFTY", candle_timestamp=_unique_ts(), setup_id="SETUP-TEST-001", previous_trade_id="PT-TEST-000", previous_exit_timestamp=_unique_ts(), seconds_since_previous_exit=120, previous_direction="BULLISH", current_direction="BULLISH", previous_regime="BULLISH", current_regime="BULLISH", same_setup_fingerprint=1, direction_changed=0, regime_changed=0, reentry_type="SAME_SETUP")
        assert result["recorded"] is True

    def test_reentry_no_classification(self):
        collector = get_collector()
        result = collector.record_reentry(trade_id="PT-TEST-002", instrument="NIFTY", candle_timestamp=_unique_ts(), setup_id="SETUP-TEST-002", reentry_type="UNKNOWN")
        assert result["recorded"] is True
...
class TestAILogging:
    ...
    def test_record_ai_call(self):
        collector = get_collector()
        result = collector.record_ai_call(instrument="NIFTY", candle_timestamp=_unique_ts(), trigger="MATERIAL_CHANGE", model="groq", provider="groq", prompt_version="5m-v1", success=1, latency_ms=150, token_usage=500)
        assert result["recorded"] is True

    def test_record_ai_call_failure(self):
        collector = get_collector()
        result = collector.record_ai_call(instrument="NIFTY", candle_timestamp=_unique_ts(), trigger="MATERIAL_CHANGE", success=0, error="LLM timeout", fallback_used=1)
        assert result["recorded"] is True

    def test_no_fake_ai_history(self):
        collector = get_collector()
        conn = sqlite3.connect(TEST_DB)
        rows = conn.execute("SELECT COUNT(*) FROM research_ai_call_log WHERE call_timestamp IS NULL").fetchone()[0]
        conn.close()
        assert rows == 0
...
class TestOutcomeTracking:
    ...
    def test_record_outcome_separate_from_decision(self):
        collector = get_collector()
        result = collector.record_outcome(outlook_id="OUT-TEST-001", setup_id="SETUP-TEST-001", instrument="NIFTY", candle_timestamp=_unique_ts(), outcome_5m="WIN", outcome_5m_timestamp=_unique_ts(), outcome_5m_price=23200, outcome_5m_return_pct=0.5, outcome_5m_direction="BULLISH")
        assert result["recorded"] is True
...
class TestDataQuality:
    ...
    def test_record_health_valid(self):
        collector = get_collector()
        result = collector.record_data_health(check_type="candle_completeness", status="VALID", instrument="NIFTY", detail="All 5m candles present")
        assert result["recorded"] is True

    def test_record_health_stale(self):
        collector = get_collector()
        result = collector.record_data_health(check_type="candle_completeness", status="STALE", instrument="NIFTY", detail="Last candle older than 10 minutes")
        assert result["recorded"] is True
...
class TestDeployValidation:
    def test_setup_identity_contains_required_fields(self):
...
class TestResearchCollector:
    ...
    def test_collect_returns_market_closed_when_not_hours(self):
        collector = get_collector()
        result = collector.collect()
        assert result["status"] == "MARKET_CLOSED"

    def test_collect_is_idempotent_on_empty_db(self):
        collector = get_collector()
        result1 = collector.collect()
        result2 = collector.collect()
        assert result1["status"] == result2["status"]

    def test_collect_returns_dict_with_required_fields(self):
        collector = get_collector()
        result = collector.collect()
...
class TestResearchCollectIntegration:
    def setup_method(self): setup_module()
    def teardown_method(self): teardown_module(); setup_module()

    def test_monitor_py_has_research_collection(self):
        monitor_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "backend", "monitor.py")
        assert os.path.exists(monitor_path)
        with open(monitor_path) as f:
            content = f.read()
        assert "research" in content.lower()
        assert "collect" in content.lower()

    def test_monitor_py_has_market_hours_check(self):
        monitor_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "backend", "monitor.py")
        with open(monitor_path) as f:
            content = f.read()
        assert "_is_market_hours" in content
        assert "run_research_collection" in content


class TestDataStatusContract:
    def test_research_collector_has_collect_method(self):
        collector = get_collector()
        assert hasattr(collector, "collect")
        assert callable(collector.collect)

    def test_collect_does_not_modify_production_tables(self):
        collector = get_collector()
        conn = sqlite3.connect(TEST_DB)
        before = {t: conn.execute(f"SELECT COUNT(*) FROM {t}").fetchone()[0] for t in ["research_setup_identity", "research_reentry_log", "research_ai_call_log", "research_outcome_tracking", "research_data_health", "research_manifest"]}
        conn.close()
        result = collector.collect()
        conn = sqlite3.connect(TEST_DB)
        after = {t: conn.execute(f"SELECT COUNT(*) FROM {t}").fetchone()[0] for t in ["research_setup_identity", "research_reentry_log", "research_ai_call_log", "research_outcome_tracking", "research_data_health", "research_manifest"]}
        conn.close()
        if result["status"] == "MARKET_CLOSED":
            assert before == after, f"Counts changed during market closed: {before} -> {after}"
